...
function usage() {
  ...
  console.error('Paste a Cookie header on stdin, or set INSTAGRAM_COOKIE_HEADER.');
}
...
const rawCookieHeader = process.env.INSTAGRAM_COOKIE_HEADER || await readStdin();
...
if (missing.length > 0) {
  ...
  console.error('Open instagram.com while logged in, copy the Cookie request header, and retry.');
  process.exit(1);
}
...
try {
  ...
  try {
    ...
    const response = await ig.request.send({ url: '/api/v1/users/web_profile_info/', qs: { username } });
    const webUser = response.body?.data?.user;
    if (!webUser?.id) {
      throw new Error('Imported cookies did not authenticate against mobile or web-profile API.');
    }
  ...
  console.error('Browser cookies were imported into the API client, but Instagram rejected them.');
